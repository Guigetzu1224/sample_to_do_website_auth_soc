...
def user_loader(username):
    ...
    req = requests.get("http://localhost:7000/lookup",json=data)
    data = req.json()
    if data['1'] == 'None':
        return
    user = User()
...
def login():
    if request.method=='GET':
        ...
        req = requests.post('http://localhost:7000/login',json=data)
        info = req.json()
...
def signup():
    if request.method=='GET':
        ...
        check_user = requests.get('http://localhost:7000/lookup/exists',json=data)
        ...
        req = requests.post('http://localhost:7000/add',json=data)
...
def show_list():
    ...
    resp = requests.get('http://localhost:6000/api/items',json=info)
    resp = resp.json()
    if flask_login.current_user.is_authenticated:
    ...
    if data['user_id'] != "None":
        ...
        fam_resp = requests.get('http://localhost:8000/family',json=temp_data)
        ...
        if 'list_of_members' in fam_resp:
            for fam_member in fam_resp['list_of_members']:
                ...
                info_2  = requests.get('http://localhost:6000/api/items',json=new_info)
                info_2 = info_2.json()
...
def add_entry():
    ...
    req=requests.post('http://localhost:6000/add',json=data)
    return redirect(url_for('show_list'))
...
def lookup():
    ...
    user_exists = bool(check_user.json()['1'])
    if user_exists:
        req = requests.get('http://localhost:6000/api/items',json=data)
        resp = req.json()
...
def delete_entry(item):
    ...
    req = requests.post('http://localhost:6000/delete',json=data)
    return redirect(url_for('show_list'))
...
def mark_as_done(item):
    if flask_login.current_user.is_authenticated:
        ...
        user_id = 'None'
    data = json.dumps({'item':item,'user_id':user_id}) 
    req = requests.post('http://localhost:6000/mark',json=data)
    return redirect(url_for('show_list'))
...
def profile(user_id):
    if flask_login.current_user.is_authenticated:
        ...
        print(user_id)
        req = requests.get('http://localhost:8000/family',json=data)
        data = req.json()
...
def family_manager(user_id):
    if flask_login.current_user.is_authenticated:
        ...
        data = req.json()
        data['user_id'] = user_id
        pend_req = requests.get('http://localhost:8000/add/pending',json=json.dumps({'user_id':user_id}))
...
def family_add(user_id):    
    if flask_login.current_user.is_authenticated:
        ...
        if user_exists:
            ...
            req = requests.post('http://localhost:8000/add',json=data)
...
def family_confirm(user_id,entry):
    if flask_login.current_user.is_authenticated:
        ...
        req = requests.post('http://localhost:8000/add/confirm',json=data)
    return redirect(url_for('show_list'))
...
def family_denied(user_id,entry):
    if flask_login.current_user.is_authenticated:
        ...
        req = requests.post('http://localhost:8000/add/delete',json=data)
        return redirect(url_for('show_list'))
